...
def wzahlen(Datenbank,lkette,maxzeit):
     ...
     try:
          with Datenbank.cursor() as readCursor,\
               ...
               logging.info(f"Typ {worttyp}: {nOffeneMld} neue Meldungen in {nAlleMld} ({100*nOffeneMld/nAlleMld:.1f} %)")
               ...
               for dat in daten:
                    ...
                    match lkette:
                         case 1:ok=w1zahlen(writeCursor,DBTW1, mld)
                         #if ok:ok=w2zahlen(writeCursor,DBTW2, mld)
                         case 3:ok=w3zahlen(writeCursor,DBTW3, mld)
                         #if ok:ok=w4zahlen(writeCursor,DBTW4, mld)
                         case 5:ok=w5zahlen(writeCursor,DBTW5, mld)
                    if ok:
          ...
          match e.errno:
               case 1062: 
                    logging.warning(f"w*zahlen: doppelter Key - Race Condition?\n{e.msg}\nwird beendet")
               case 1064: 
